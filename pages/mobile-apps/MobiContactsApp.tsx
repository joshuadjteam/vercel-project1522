    ...
    return (
        <div className="w-full h-full flex flex-col bg-gray-100 dark:bg-gray-800 text-light-text dark:text-dark-text">
            <header className="p-4 flex-shrink-0 bg-white dark:bg-gray-900 shadow-md flex justify-between items-center">
                ...
                <button onClick={handleAddContact} className="p-2 rounded-full bg-blue-600 text-white"><UserPlusIcon /></button>
            </header>
            <main className="flex-grow overflow-y-auto">
                {isLoading ? <p className="p-4 text-center">Loading contacts...</p> : contacts.length > 0 ? (
                    <ul>
                        {contacts.map(contact => (
                            <li key={contact.id} className="border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        {contact.email && <p className="text-sm text-gray-500 dark:text-gray-400 truncate max-w-[150px]">{contact.email}</p>}
                                    </div>
                                </div>
                                <div className="flex space-x-2">
                                    <button onClick={() => handleEditContact(contact)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-blue-500" title="Edit"><PencilIcon /></button>
                                    <button onClick={() => handleDeleteContact(contact.id)} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-red-500" title="Delete"><TrashIcon /></button>
                                </div>
                            </li>
                        ))}
                    </ul>
