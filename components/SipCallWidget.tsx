import React, { useEffect, useRef, useState } from 'react';
import { useSip } from '../hooks/useSip';

const MuteIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>;
const UnmuteIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5.586 15.586a3 3 0 014.242 0L12 17.757l2.172-2.171a3 3 0 014.242 0M9 12a3 3 0 116 0v6a3 3 0 01-6 0v-6z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15.586 15.586a3 3 0 010 4.243L12 23.07l-3.586-3.24a3 3 0 010-4.243m5.172-2.171a3 3 0 010-4.243L12 2.93 8.414 6.171a3 3 0 010 4.243" /></svg>;
const EndCallIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="currentColor" viewBox="0 0 24 24"><path d="M12.01,16.46c-2.3,0-4.52-0.62-6.52-1.75l-2.12,2.12c-0.39,0.39-1.02,0.39-1.41,0l-1.42-1.42c-0.39-0.39-0.39-1.02,0-1.41l2.12-2.12C1.62,10.04,1,7.82,1,5.52c0-0.41,0.34-0.75,0.75-0.75h4c0.35,0,0.66,0.24,0.74,0.58l0.85,3.83c0.07,0.32-0.01,0.66-0.23,0.9L5.5,11.53c0.95,1.86,2.5,3.4,4.37,4.37l1.45-1.45c0.23-0.23,0.58-0.3,0.9-0.23l3.83,0.85c0.34,0.08,0.58,0.39,0.58,0.74v4c0,0.41-0.34,0.75-0.75,0.75C17.3,21,14.67,20.08,12.01,16.46z" transform="rotate(-135 12 12)" /></svg>;
const AcceptIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="currentColor" viewBox="0 0 24 24"><path d="M12.01,16.46c-2.3,0-4.52-0.62-6.52-1.75l-2.12,2.12c-0.39,0.39-1.02,0.39-1.41,0l-1.42-1.42c-0.39-0.39-0.39-1.02,0-1.41l2.12-2.12C1.62,10.04,1,7.82,1,5.52c0-0.41,0.34-0.75,0.75-0.75h4c0.35,0,0.66,0.24,0.74,0.58l0.85,3.83c0.07,0.32-0.01,0.66-0.23,0.9L5.5,11.53c0.95,1.86,2.5,3.4,4.37,4.37l1.45-1.45c0.23-0.23,0.58-0.3,0.9-0.23l3.83,0.85c0.34,0.08,0.58,0.39,0.58,0.74v4c0,0.41-0.34,0.75-0.75,0.75C17.3,21,14.67,20.08,12.01,16.46z" /></svg>;

interface SlideToInteractProps {
    onAccept: () => void;
    onDecline: () => void;
}

const SlideToInteract: React.FC<SlideToInteractProps> = ({ onAccept, onDecline }) => {
    const sliderRef = useRef<HTMLButtonElement>(null);
    const containerRef = useRef<HTMLDivElement>(null);
    const [isDragging, setIsDragging] = useState(false);
    const [sliderPos, setSliderPos] = useState(0);

    const handleInteractionStart = (e: React.MouseEvent | React.TouchEvent) => {
        setIsDragging(true);
    };

    const handleInteractionMove = (e: React.MouseEvent | React.TouchEvent) => {
        if (!isDragging || !sliderRef.current || !containerRef.current) return;
        const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY;
        const rect = containerRef.current.getBoundingClientRect();
        const newY = clientY - rect.top - (sliderRef.current.offsetHeight / 2);
        
        const containerHeight = containerRef.current.offsetHeight;
        const maxUp = -containerHeight * 0.4;
        const maxDown = containerHeight * 0.4;

        if (newY < maxUp) { onAccept(); setIsDragging(false); setSliderPos(0); }
        else if (newY > maxDown) { onDecline(); setIsDragging(false); setSliderPos(0); }
        else { setSliderPos(newY); }
    };

    const handleInteractionEnd = () => {
        setIsDragging(false);
        setSliderPos(0);
    };

    return (
        <div ref={containerRef} className="relative w-full h-full flex flex-col items-center justify-between text-white"
            onMouseMove={handleInteractionMove} onMouseUp={handleInteractionEnd} onMouseLeave={handleInteractionEnd}
            onTouchMove={handleInteractionMove} onTouchEnd={handleInteractionEnd}>

            <div className="flex flex-col items-center opacity-80 animate-bounce">
                <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
                <span className="font-semibold">Accept</span>
            </div>

            <button
                ref={sliderRef}
                className="w-20 h-20 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center cursor-grab active:cursor-grabbing transition-transform duration-100"
                style={{ transform: `translateY(${sliderPos}px)` }}
                onMouseDown={handleInteractionStart}
                onTouchStart={handleInteractionStart}
            >
                <AcceptIcon />
            </button>

            <div className="flex flex-col items-center opacity-80 animate-bounce" style={{animationDirection: 'reverse'}}>
                <span className="font-semibold">Decline</span>
                <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </div>
        </div>
    );
};


const SipCallWidget: React.FC = () => {
    const { callState, remoteIdentity, remoteStream, isMuted, toggleMute, answerCall, hangupCall, callDuration } = useSip();
    const remoteVideoRef = useRef<HTMLVideoElement>(null);
    const [showUnmuteButton, setShowUnmuteButton] = useState(false);

    useEffect(() => {
        const video = remoteVideoRef.current;
        if (video && remoteStream) {
            video.srcObject = remoteStream;
            setShowUnmuteButton(false);
            video.play().catch(() => { video.muted = true; setShowUnmuteButton(true); });
        }
    }, [remoteStream]);

    const handleManualUnmute = () => {
        if (remoteVideoRef.current) {
            remoteVideoRef.current.muted = false;
            remoteVideoRef.current.play().then(() => setShowUnmuteButton(false));
        }
    };
    
    if (callState === 'Idle') return null;

    const formatDuration = (s: number) => `${Math.floor(s / 60).toString().padStart(2, '0')}:${Math.floor(s % 60).toString().padStart(2, '0')}`;
    const getStatusText = () => callState === 'Active' ? formatDuration(callDuration) : callState;

    return (
        <div className="fixed inset-0 bg-gray-900 z-[100] animate-fade-in flex flex-col">
            <div className="relative flex-grow w-full h-full flex items-center justify-center">
                <video ref={remoteVideoRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-black/10"></div>
                
                {showUnmuteButton && (
                    <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center animate-fade-in">
                        <p className="text-lg mb-4 text-white">Remote audio muted by browser</p>
                        <button onClick={handleManualUnmute} className="px-6 py-3 bg-blue-600 rounded-lg font-semibold text-white">Tap to Unmute</button>
                    </div>
                )}
            </div>

            <div className="absolute top-0 left-0 right-0 p-6 text-center text-white z-10">
                <h2 className="text-4xl font-light">{remoteIdentity || "Unknown Caller"}</h2>
                <p className="text-xl text-yellow-400 mt-2">{getStatusText()}</p>
            </div>

            <div className="absolute bottom-0 left-0 right-0 p-8 z-10">
                {callState === 'Incoming' ? (
                    <div className="h-64">
                         <SlideToInteract onAccept={answerCall} onDecline={hangupCall} />
                    </div>
                ) : (
                    <div className="flex justify-center items-center space-x-8">
                        <button onClick={toggleMute} className={`h-16 w-16 rounded-full flex items-center justify-center transition-colors ${isMuted ? 'bg-yellow-500' : 'bg-white/20'}`}>
                            {isMuted ? <UnmuteIcon /> : <MuteIcon />}
                        </button>
                        <button onClick={hangupCall} className="h-20 w-20 rounded-full flex items-center justify-center bg-red-600">
                            <EndCallIcon />
                        </button>
                    </div>
                )}
            </div>
        </div>
    );
};

export default SipCallWidget;