...
const ConsoleSwitchApp: React.FC<ConsoleSwitchAppProps> = ({ closeWindow }) => {
    ...
    return (
        <AppContainer className="w-full h-full p-8 text-light-text dark:text-white flex flex-col items-center">
            <h1 className="text-4xl font-bold mb-4">Console Switcher</h1>
            <p className="text-gray-600 dark:text-gray-300 mb-8">Choose your preferred console experience.</p>

            <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {consoles.map(c => (
                    <div key={c.id} className={`p-6 rounded-lg border-2 transition-all ${view === c.id ? 'border-blue-500 bg-blue-500/10' : 'border-gray-300 dark:border-slate-700'}`}>
                        <div className="flex items-center space-x-4 mb-4">
                            {c.icon}
                            <h2 className="text-2xl font-semibold">{c.name}</h2>
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 min-h-[60px]">{c.description}</p>
                        <button
                            onClick={() => handleSelect(c.id as any)}
                            disabled={view === c.id}
                            className="w-full py-2 px-4 rounded-md font-semibold transition-colors disabled:bg-blue-600 disabled:text-white disabled:cursor-not-allowed bg-gray-200 dark:bg-slate-700 hover:bg-blue-500 hover:text-white"
                        >
