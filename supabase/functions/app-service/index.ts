...
const getChatId = (userId1: number, userId2: number): string => {
  ...
  try {
    ...
    switch (resource) {
      ...
      case 'notes':
        switch (action) {
          case 'get':
            ({ data, error } = await supabaseAdmin.from('notes').select('*').eq('owner', userProfile.username).order('created_at', { ascending: false }));
            if (error) throw error;
            return new Response(JSON.stringify({ notes: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'add':
            ({ data, error } = await supabaseAdmin.from('notes').insert({ title: payload.title, content: payload.content, owner: userProfile.username }).select().single());
            if (error) throw error;
            return new Response(JSON.stringify({ note: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'update':
            ({ data, error } = await supabaseAdmin.from('notes').update({ title: payload.title, content: payload.content }).eq('id', payload.id).eq('owner', userProfile.username).select().single());
            if (error) throw error;
            return new Response(JSON.stringify({ note: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'delete':
          ...
          default: throw new Error('Invalid action for notes');
        }
      ...
      case 'mails':
        switch (action) {
          case 'get':
            ({ data, error } = await supabaseAdmin.from('mails').select('*').or(`recipient.eq.${userProfile.username},sender.eq.${userProfile.username}`).order('timestamp', { ascending: false }));
            if (error) throw error;
            return new Response(JSON.stringify({ mails: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'send':
            ({ data, error } = await supabaseAdmin.from('mails').insert({ ...payload, sender: userProfile.username }).select().single());
            if (error) throw error;
            return new Response(JSON.stringify({ mail: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'markAsRead':
            ({ error } = await supabaseAdmin.from('mails').update({ read: true }).eq('id', payload.id).eq('recipient', userProfile.username));
            if (error) throw error;
            return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          case 'delete':
            ...
            if (error) throw error;
            return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          default: throw new Error('Invalid action for mails');
        }
      ...
      case 'contacts':
        switch (action) {
          ...
          case 'add':
            ({ data, error } = await supabaseAdmin.from('contacts').insert({ ...payload, owner: userProfile.username }).select().single());
            if (error) throw error;
          ...
          case 'delete':
            ...
            if (error) throw error;
            return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
          default: throw new Error('Invalid action for contacts');
        }
